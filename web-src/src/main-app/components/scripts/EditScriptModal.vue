<template>
  <div v-if="visible" class="modal-overlay">
    <div class="modal-content edit-script-modal card">
      <div class="modal-header">
        <span class="modal-title">Edit Script</span>
      </div>
      ...
      <div class="modal-body">
        <div v-if="loading" class="loading-state">
          ...
          <span>Loading script configuration...</span>
        </div>
        <div v-else-if="loadingError" class="error">{{ loadingError }}</div>
...
import {
  ...
  workDirField
} from '@/admin/components/scripts-config/script-fields';

export default {
  name: 'EditScriptModal',
  components: {
    PromisableButton,
    ScriptParamList,
    ScriptPathField,
  ...
  computed: {
    ...
    loading() {
      return this.visible && !this.scriptConfig && !this.loadingError;
    },
    isDirty() {
  ...
  watch: {
    visible(newVal) {
      if (newVal) {
        ...
        this.activeTab = 0;
        if (this.scriptName) {
          this.$store.dispatch(`${this.storeModule}/init`, this.scriptName);
        }
    ...
    scriptConfig: {
      ...
      handler(config) {
        if (config) {
          // Capture original script path on first load
          if (!this.originalScriptPath && config.script_path) {
            this.originalScriptPath = config.script_path;
          }
    ...
    scriptName(newVal) {
      // Reset original path when script changes
      this.originalScriptPath = null;
      if (newVal && this.visible) {
        this.$store.dispatch(`${this.storeModule}/init`, newVal);
      }
    },
